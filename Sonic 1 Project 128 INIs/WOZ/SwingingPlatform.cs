...
namespace S12005ObjectDefinitions.GHZ
{
    class SwingingPlatform : ObjectDefinition
    {
        ...
        {
            byte[] artfile = ObjectHelper.OpenArtFile("../artnem/wozswing.bin", CompressionType.Nemesis);
            img = ObjectHelper.MapASMToBmp(artfile, "../_maps/obj15woz.asm", 0, 2);
            for (int i = 0; i < labels.Length; i++)
                imgs.Add(ObjectHelper.MapASMToBmp(artfile, "../_maps/obj15woz.asm", labels[i], i == 1 ? 1 : 2));
        }
